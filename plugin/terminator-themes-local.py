...
AVAILABLE = ['TerminatorThemesLocal']

class TerminatorThemesLocal(plugin.Plugin):

    capabilities = ['terminal_menu']
    config_base = ConfigBase()
    base_url = os.path.expanduser('~') + '/.config/terminator/themes/terminator-themes.json'
    ...
    def callback(self, menuitems, menu, terminal):
        ...
        item.set_label("Themes (local)")
        item.set_sensitive(True)
        menuitems.append(item)

    def configure(self, widget, data = None):
        ui = {}
        dbox = Gtk.Dialog( _("Terminator themes (local)"), None, Gtk.DialogFlags.MODAL)

        local_file = open(self.base_url, 'r')
        self.themes_from_repo = json.loads(local_file.read())["themes"]
        local_file.close()
